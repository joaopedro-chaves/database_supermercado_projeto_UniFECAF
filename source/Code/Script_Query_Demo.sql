...
SELECT p.nome_produto AS 'Produto', p.categoria_produto AS 'Categoria', e.lote_produto AS 'Lote', e.quantidade_estoque AS 'Qtd em Estoque', DATE_FORMAT(e.data_validade, '%d/%m/%Y') AS 'Data de Validade'
FROM tbl_estoque e
...
SELECT c.nome_colaborador AS 'Colaborador', COUNT(v.id_vendas) AS 'Total de Vendas', CONCAT(
        'R$ ',
        REPLACE (
                FORMAT(SUM(v.valor_total), 2), ',', '.'
            )
